...
void trainingSample(double values[], double expectedValues[]) {
...
void runNetwork(double weights[][arraySize], double values[], double thresholds[]) {
...
double updateWeights(double weights[][arraySize], double values[], double expectedValues[], double thresholds[]) {
    ...
    for (int i = 1 + inputNodes + hiddenNodes; i <= nodes; i++) {
        ...
        for (int j = 1 + inputNodes; j <= 1 + inputNodes + hiddenNodes; j++) {
            ...
            double delta = learningRate * values[j] * outputErrorGradient;
